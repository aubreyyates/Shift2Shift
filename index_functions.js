        ...
        $.post('load_messages_objects.php', {message_load:message_load}, function(result) {
            ...
            messages = JSON.parse(result)
             ...
             for (i=0; i<messages.length;i++) {
                ...
                messages[i].message = DOMPurify.sanitize(messages[i].message);
                messages[i].email = DOMPurify.sanitize(messages[i].email);
                messages[i].new_date = DOMPurify.sanitize(messages[i].new_date);
            }           
       ...
       $( "#exit_account_modal" ).click(function() {  
        ...
        $( "#save_settings" ).click(function() {  
            ...
            if (settings == 'info') {
                ...
                if (!company_name.match(letters_company)) {
                    // Alert that company name is invalid
                    alert("Your company can only have letters, numbers, spaces, hyphens, and ampersands in it's name.")
                    //Leave the function
                    return
                }
                // Check for company name length
                if (company_name.length > 99) {
                    // Alert that company name is too long
                    alert("Your company name must be less than 100 charaters.")
                    //Leave the function
                    return
                }
                ...
                $.post('update_company_settings_info.php', {company_name:company_name,company_website:company_website,company_color:company_color});
