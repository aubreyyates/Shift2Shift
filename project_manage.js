...
$(document).ready(function () {
    ...
    $('#sorting').change(function () {
        ...
        } else if ($('#sorting').val() == 'Date Create') {
    ...
    $("#export").click(function () {
        ...
        if (export_type == 'csv') {
            ...
            // Convert JSON to CSV 
            csvFirstLine = 'Id, First, Last, Date, Start Time, AM/PM, End Time, AM/PM, Length, Description\r\n'
            ...
            if (window.navigator.msSaveOrOpenBlob)  // IE hack; see http://msdn.microsoft.com/en-us/library/ie/hh779016.aspx
                window.navigator.msSaveBlob(blob, "project_data.csv");
            else {
            ...
            var headers = ["Id", "First", "Last", "Date", "Start Time", "AM/PM", "End Time", "AM/PM", "Length", "Description"];
            ...
            if (arr.length == 0) {
                if (filter_date == false) {
                    for (var i = 0; i < entries.length; i++) {
                        ...
                        excel.set(0, 1, i + 1, entries[i].first);
                        excel.set(0, 2, i + 1, entries[i].last);
                        excel.set(0, 3, i + 1, entries[i].date);
                        excel.set(0, 4, i + 1, entries[i].start);
                        excel.set(0, 5, i + 1, entries[i].startdiem);
                        excel.set(0, 6, i + 1, entries[i].end);
                        excel.set(0, 7, i + 1, entries[i].enddiem);
                        excel.set(0, 8, i + 1, entries[i].time);
                        excel.set(0, 9, i + 1, entries[i].description);
                    }
                } else {
                    for (var i = 0; i < entries.length; i++) {
                        if (moment(entries[i].date).unix() >= date.unix() && moment(entries[i].date).unix() < date_end.unix()) {
                            excel.set(0, 0, c + 1, entries[i].id);
                            excel.set(0, 1, c + 1, entries[i].first);
                            excel.set(0, 2, c + 1, entries[i].last);
                            excel.set(0, 3, c + 1, entries[i].date);
                            excel.set(0, 4, c + 1, entries[i].start);
                            excel.set(0, 5, c + 1, entries[i].startdiem);
                            excel.set(0, 6, c + 1, entries[i].end);
                            excel.set(0, 7, c + 1, entries[i].enddiem);
                            excel.set(0, 8, c + 1, entries[i].time);
                            excel.set(0, 9, c + 1, entries[i].description);
                            c += 1
                        }
                    }
                }
            ...
            doc.setFontSize(8);
            doc.text('ID', 10, 10)
            doc.text('First', 20, 10)
            doc.text('Last', 45, 10)
            doc.text('E-mail', 70, 10)
            doc.text('Date', 95, 10)
            doc.text('Start', 115, 10)
            doc.text('End', 135, 10)
            doc.text('Length', 155, 10)
            var c = 0
            if (arr.length == 0) {
                if (filter_date == false) {
                    for (var i = 0; i < entries.length; i++) {
                        doc.text(entries[i].id, 10, i * 5 + 20)
                        doc.text(entries[i].first, 20, i * 5 + 20)
                        doc.text(entries[i].last, 45, i * 5 + 20)
                        //doc.text(entries[i].email, 70,i*5+20)                               
                        doc.text(entries[i].date, 95, i * 5 + 20)
                        doc.text(entries[i].start.substring(0, 10), 115, i * 5 + 20)
                        doc.text(entries[i].startdiem, 123, i * 5 + 20)
                        doc.text(entries[i].end, 135, i * 5 + 20)
                        doc.text(entries[i].enddiem.substring(0, 10), 143, i * 5 + 20)
                        doc.text(entries[i].time, 155, i * 5 + 20)
                    ...
                    for (var i = 0; i < entries.length; i++) {
                        if (moment(entries[i].date).unix() >= date.unix() && moment(entries[i].date).unix() < date_end.unix()) {
                            ...
                            doc.text(entries[i].first, 20, c * 5 + 20)
                            doc.text(entries[i].last, 45, c * 5 + 20)
                            //doc.text(entries[i].email, 70,c*5+20)                               
                            doc.text(entries[i].date, 95, c * 5 + 20)
                            doc.text(entries[i].start.substring(0, 10), 115, c * 5 + 20)
                            doc.text(entries[i].startdiem, 123, c * 5 + 20)
                            doc.text(entries[i].end, 135, c * 5 + 20)
                            doc.text(entries[i].enddiem.substring(0, 10), 143, c * 5 + 20)
                            doc.text(entries[i].time, 155, c * 5 + 20)
                            c += 1
                        }
                    }
                }
                ...
                for (var i = 0; i < entries.length; i++) {
                    if (entries[i].id == arr[c]) {
                        ...
                        doc.text(entries[i].first, 20, c * 5 + 20)
                        doc.text(entries[i].last, 45, c * 5 + 20)
                        doc.text(entries[i].date, 70, c * 5 + 20)
                        doc.text(entries[i].start.substring(0, 10), 90, c * 5 + 20)
                        doc.text(entries[i].startdiem, 98, c * 5 + 20)
                        doc.text(entries[i].end, 110, c * 5 + 20)
                        doc.text(entries[i].enddiem.substring(0, 10), 118, c * 5 + 20)
                        doc.text(entries[i].time, 130, c * 5 + 20)
                        c += 1
                    }
                }
            }
...
function prepare_entry_line(first, last, date, time, id, start, startdiem, end, enddiem, description) {
    ...
    line_data = " First Name: " + first + " | Last Name: " + last + " | Date: " + date + " | Time: " + time
    // Create more html
    ...
    if (len > 450) {
        ...
        line_data += "..."
        // Add class
        text_1 = "<div class='entry_template'> <div id='entry_box' class='entry_line'><div id='entry_text' class='long_text' data-first='" + first + "' data-last='" + last + "' data-date='" + date + "' data-time='" + time + "' style='float:left;margin-left:12px;font-size:16px;'>"
    }
...
function date_compare() {
    ...
    for (i = vnum_s; i < max_view; i++) {
        ...
        // Get the html for the entry
        text = prepare_entry_line(search_objects[i].first, search_objects[i].last, search_objects[i].date, search_objects[i].time, search_objects[i].id, search_objects[i].start, search_objects[i].startdiem, search_objects[i].end, search_objects[i].enddiem, search_objects[i].description)
        // Put the entry on #list
        var entry = $('#list').append(text)
...
$('#list').on('mouseenter', '.long_text', function () {
    ...
    $('#info_description_long').text("First Name: " + first + " | Last Name: " + last + " | Date: " + date + " | Time: " + time)
    // Get the height
