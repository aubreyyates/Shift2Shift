<?php
    ...
    if (isset($_SESSION['u_id']) && isset($_POST['update'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM managers WHERE manager_email = ?;");
        $stmt->bind_param("s",$email);
        $stmt->execute();
        $result = $stmt->get_result();        
        $resultCheck = mysqli_num_rows($result);
        ...
        $stmt = $conn->prepare("SELECT * FROM employees WHERE emp_email = ? AND emp_id!=?;");
        ...
        // Update the name
        $stmt = $conn->prepare("UPDATE employees SET emp_first=? WHERE emp_id=? AND emp_org=?;");
        // Put variables into statement
        $stmt->bind_param("sii",$first,$id,$org_id);
        // Execute SQL
        $stmt->execute();
        // Update the date
        $stmt = $conn->prepare("UPDATE employees SET emp_last=? WHERE emp_id=? AND emp_org=?;");
        // Put variables into statement
        $stmt->bind_param("sii",$last,$id,$org_id);
        // Execute SQL
        $stmt->execute();   
        // // Update the date
        $stmt = $conn->prepare("UPDATE employees SET emp_email=? WHERE emp_id=? AND emp_org=?;");
        // Put variables into statement
